...
CREATE POLICY byred_tenants_select_member
  ...
  TO authenticated
  USING (
    public.byred_is_member_of_tenant(public.byred_tenants.id)
  );
...
CREATE POLICY byred_tenants_update_admin
  ...
  FOR UPDATE
  TO authenticated
  USING (public.byred_is_admin_for_tenant(public.byred_tenants.id))
  WITH CHECK (public.byred_is_admin_for_tenant(public.byred_tenants.id));
