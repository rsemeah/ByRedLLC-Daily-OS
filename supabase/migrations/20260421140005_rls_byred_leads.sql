...
CREATE POLICY byred_leads_select_member
  ...
  TO authenticated
  USING (
    public.byred_is_member_of_tenant(public.byred_leads.tenant_id)
  );
...
CREATE POLICY byred_leads_insert_member_active_tenant
  ...
  WITH CHECK (
    ...
    AND (
      ...
      OR public.byred_leads.tenant_id = public.byred_jwt_active_tenant_id()
    )
  );
...
CREATE POLICY byred_leads_update_member_admin_or_owner
  ...
  USING (
    public.byred_is_member_of_tenant(public.byred_leads.tenant_id)
    AND (
      public.byred_is_admin_for_tenant(public.byred_leads.tenant_id)
      OR public.byred_leads.created_by_user_id = public.byred_current_user_id()::uuid
      OR public.byred_leads.assigned_user_id = public.byred_current_user_id()::uuid
    )
  )
  WITH CHECK (
    public.byred_is_member_of_tenant(public.byred_leads.tenant_id)
