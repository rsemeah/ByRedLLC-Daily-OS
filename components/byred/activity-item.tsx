    ...
    <div className="flex gap-3 py-3">
      ...
      <div className="flex-1 min-w-0 pt-0.5">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-zinc-400 font-mono">{activity.type}</span>
        </div>
        {activity.summary && (
          <p className="text-sm text-zinc-600 mt-1">{activity.summary}</p>
        )}
        <div className="flex items-center gap-2 mt-1">
          <span className="text-xs text-zinc-400">{relativeTime}</span>
          {showObject && (
            <Link href={objectHref} className="text-xs text-zinc-400 hover:text-zinc-700 underline-offset-2 hover:underline">
