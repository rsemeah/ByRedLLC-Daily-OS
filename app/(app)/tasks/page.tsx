...
export default function TasksPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tenantFilter !== 'all') tasks = tasks.filter((t) => t.tenant_id === tenantFilter)
    if (statusFilter !== 'all') tasks = tasks.filter((t) => t.status === statusFilter)
    if (aiModeFilter !== 'all') tasks = tasks.filter((t) => t.ai_mode === aiModeFilter)
    ...
    return tasks
  }, [search, tenantFilter, statusFilter, aiModeFilter, priorityFilter, showOverdue, showBlockers])

  const hasFilters =
    search || tenantFilter !== 'all' || statusFilter !== 'all' || aiModeFilter !== 'all' ||
    priorityFilter !== 'all' || showOverdue || showBlockers
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button asChild className="bg-byred-red hover:bg-byred-red-hot text-white gap-2">
          <Link href="/tasks/new">
