...
/**
 * Structured JSON logger. One line per event, parseable by any log pipeline
 * (Axiom, Datadog, Vercel log drains, grep). Wraps console.* so we inherit
 * Vercel + local dev integration without a dependency.
 *
 * Usage:
 *   const log = logger.child({ component: "monday_pull", request_id: "…" })
 *   log.info("sync_started", { tenants: 8 })
 *   log.warn("rate_limited", { retry_after_ms: 2000 })
 *   log.error("sync_failed", { error: e.message }, e)
 *
 * Every call emits a flat JSON object with a standard envelope:
 *   { ts, level, event, ...component_ctx, ...call_ctx, err? }
 */
...
/**
 * Pull a correlation id out of common hosting headers; fall back to a fresh UUID.
 */
